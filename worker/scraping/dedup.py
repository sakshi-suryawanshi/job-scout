...
def is_globally_remote(job: Dict) -> bool:
    ...
    # Hard reject: region-locked
    if _REJECT_PATTERNS.search(text):
        return False

    # Hard reject: India-based location (not in title/description, just location field)
    if _INDIA_LOCATIONS.search(location):
        return False

    # Explicit global acceptance
    if _GLOBAL_ACCEPT.search(text):
        return True

    # If location is just "Remote" or empty and job is marked remote, accept
    loc_lower = location.lower().strip()
    if job.get("is_remote") and loc_lower in ("", "remote", "remote job", "unknown"):
        return True

    # If marked remote but has a specific non-India location, accept
    # (e.g., "Remote - San Francisco" means the company is in SF but role is remote)
