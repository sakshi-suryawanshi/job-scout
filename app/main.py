...
def load_last_pipeline_run():
    try:
        result = db._request("GET", "pipeline_runs", params={"order": "started_at.desc", "limit": 1})
...
with left:
    ...
    if last_run:
        ...
        if stats:
            ...
            s1.metric("Found",          stats.get("jobs_found", 0))
            s2.metric("Applied",        stats.get("auto_applied", 0))
            s3.metric("Need attention", stats.get("needs_attention", 0))
    else:
        st.info("No pipeline runs yet. Go to Auto-Pilot to configure.")
...
with right:
    ...
    if quota:
        ...
        st.caption("Quota data will appear here after the first pipeline run.")
